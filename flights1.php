...
<head>
    ...
    <style>
        * {
        ...
        .modal-content .summary p strong {
            color: #0288d1;
        }
...
<div class="container">
    ...
    foreach ($flights as $flight_id => $flight) {
        ...
        // Seat selection form
        echo "<form id='booking-form-$flight_id' action='book_flight.php' method='POST'>";
        ...
        // Luggage Space Pre-Reservation Section
        ...
        echo "<button type='button' class='book-button' onclick='showPassengerModal($flight_id)'>Book Now</button>";
        echo "</form>";
        echo "</div>";
    }
    $conn->close();
...
<div class="modal" id="passenger-modal">
    <div class="modal-content">
        <span class="close-modal" onclick="closePassengerModal()">×</span>
        <h3>Passenger Details</h3>
        <div class="summary" id="booking-summary">
            <p><strong>Selected Class:</strong> <span id="summary-class"></span></p>
            <p><strong>Selected Seat:</strong> <span id="summary-seat"></span></p>
            <p><strong>Premium Seat:</strong> <span id="summary-premium"></span></p>
            <p><strong>Luggage Bin:</strong> <span id="summary-bin"></span></p>
        </div>
...
<script>
    ...
    function showSeats(flightId, classType) {
        ...
        fetch(`get_seats.php?flightId=${flightId}&classType=${classType}`)
            ...
            .then(seats => {
                ...
                let standardHtml = '<h4>Standard Seats</h4>';
                standardHtml += '<div class="seat-grid">';
                let premiumHtml = '';
    ...
    function togglePremiumSeats(flightId) {
        ...
        if (isActive) {
            premiumSeatsDiv.style.display = 'block';
            premiumEnabledInput.value = '1';
        } else {
            premiumSeatsDiv.style.display = 'none';
            premiumEnabledInput.value = '0';
            const premiumSeats = document.querySelectorAll(`#premium-seat-grid-${flightId} .seat`);
            premiumSeats.forEach(seat => seat.classList.remove('selected'));
    ...
    function toggleLuggageOptions(flightId) {
        ...
        if (isActive) {
            luggageOptionsDiv.style.display = 'block';
            luggageEnabledInput.value = '1';
        } else {
            luggageOptionsDiv.style.display = 'none';
            luggageEnabledInput.value = '0';
    ...
    function showPassengerModal(flightId) {
        ...
        const premiumEnabled = document.getElementById(`premium-enabled-${flightId}`).value === '1';
        const luggageEnabled = document.getElementById(`luggage-enabled-${flightId}`).value === '1';
        const selectedBin = document.getElementById(`selected-bin-${flightId}`).value;
        ...
        document.getElementById('summary-class').textContent = selectedClass;
        document.getElementById('summary-seat').textContent = selectedSeat;
        document.getElementById('summary-premium').textContent = premiumEnabled ? 'Yes (₹2000)' : 'No';
        document.getElementById('summary-bin').textContent = luggageEnabled && selectedBin ? selectedBin : 'Not Selected';
    ...
    function confirmBooking() {
        ...
        if (!passengerName || !passengerAge || !passengerGender || !paymentMethod) {
            alert('Please fill in all passenger details and select a payment method.');
            return;
        }

        const form = document.getElementById(`booking-form-${currentFlightId}`);
        const formData = new FormData(form);

        formData.append('passenger_name', passengerName);
        formData.append('passenger_age', passengerAge);
        formData.append('passenger_gender', passengerGender);
    ...
    function sendMessage() {
        ...
        setTimeout(() => {
            ...
            if (lowerMessage.includes('flight status')) {
                botResponse = 'I can check the flight status for you! Please provide your flight number (e.g., AI123).';
            } else if (lowerMessage.includes('seat')) {
                botResponse = 'You can select a seat from the seat map above. Would you like to unlock premium seats for extra comfort? Click the "Unlock Premium Seats" button!';
            } else if (lowerMessage.includes('luggage') || lowerMessage.includes('bin')) {
                botResponse = 'You can reserve overhead bin space for your carry-on by clicking the "Reserve Luggage Space" button. It’s only ₹300!';
            } else if (lowerMessage.includes('recommend')) {
                botResponse = 'I recommend reserving a bin slot if you have important carry-on items. Also, premium seats offer extra comfort—would you like to unlock them?';
            } else if (lowerMessage.includes('ai123')) {
                botResponse = 'Flight AI123 from Delhi to Mumbai is on time, departing at 10:00 AM. Would you like to reserve a seat or bin slot for this flight?';
            } else {
                botResponse = 'I’m here to help! You can ask about flight status, seat selection, luggage space, or get recommendations. What would you like to know? ✈️';
            }
